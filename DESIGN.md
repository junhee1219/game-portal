...
```sql
CREATE TABLE users (
  id            VARCHAR(64)  NOT NULL PRIMARY KEY,        -- uuid str
  login_id      VARCHAR(32)  NULL,                        -- UNIQUE. nullable인 이유: 카카오-only 가입 대비
  password_hash VARCHAR(128) NULL,
  kakao_id      VARCHAR(32)  NULL,                        -- UNIQUE. 자리만 (지금 미사용)
  nickname      VARCHAR(32)  NOT NULL,                    -- 표시명 (가변). login_id는 불변 로그인 키
...
| Method | Endpoint | 동작 |
|---|---|---|
| POST `/auth/register` | `{login_id, password, nickname, visitor_id}` | bcrypt 해싱 → User insert → claim → 세션 쿠키 |
| POST `/auth/login` | `{login_id, password, visitor_id}` | verify → claim → 세션 쿠키 |
...
- brute-force: in-memory `IP+login_id 5분 5회 → 429`. redis/slowapi 과설계 금지.
- **비번 분실 = 복구 없음 (명시적 트레이드오프).** 이메일 인프라는 YAGNI. 새 계정 파면 되고 기록은 DB에 남는다.
- 실패 메시지는 가입/로그인 공통 모호하게 ("아이디 또는 비밀번호를 확인하세요").
...
### 3.1 merge 시맨틱 — 키별 선언 (LWW 통일 금지)

"update_time 비교 양방향 sync"는 설정값엔 맞지만 신기록엔 틀린다: 오래 꺼져있던 폰 B가
화요일에 best=40을 쓰면 (월요일의 서버 best=120보다) timestamp가 더 새것이라 LWW가 기록을 파괴한다.
게다가 클라이언트 시계는 못 믿는다. 그래서 키별 merge 규칙:

| merge | 의미 | 대상 | 구현 |
|---|---|---|---|
| `max` | 숫자, 클수록 좋음 | 신기록, 진행 레벨 | `GREATEST(server, client)` — 시계 불필요, 순서 무관, 멱등 |
| `union` | 객체, 키별 max | vaseStars `{레벨: 별}` | 키 합집합 + 값은 max |
| `union_min` | 객체, 키별 **min** | vaseBest `{레벨: moves}` — **moves는 적을수록 좋음** | 키 합집합 + 값은 min |
...
JSON.parse throw → 레벨별 기록 맵 전체 소실). 게임별 커스텀 merge 함수는 여전히 안 만든다 — 4종이 선언적 한계.
...
append-only ledger. **클라이언트발 "credit earn" 이벤트는 만들지 않는다** — 서버가 이미 받는
...
- balance = `SUM(amount)` 파생 (캐시 컬럼 X — lost update race 원천 제거, cap이 행 수를 눌러줌).
- 비로그인 적립은 visitor 단위로 쌓이고 claim 때 1회 백필 (1.3의 ③) — scores와 동일한 단일 규칙.
- `GET /api/me/credits` — 잔액 + 최근 내역. rank/헤더에 배지 (fetch 실패 시 숨김).
- duration 위조는 하한+clamp+cap 3중으로 가둠. cap 한도 내 위조는 캐주얼 포털 수준에서 수용.
...
| **3. 크레딧** | ledger + ping/score 적립 훅 + 잔액 배지 | 서버 전용. spend는 용도 확정 때 |
