...
- **즉시성이 깊이보다 먼저다 (2026-07-09 학습 — 1라운드 3종 전멸에서).** brew(엔진빌더)·ward(턴제전술)·chain(물리빌더) 세 프로토타입이 전부 "뭘 어쩌라는 건지 모르겠다"로 폐기됐다. 셋 다 "깊지만 즉시적이지 않은" 게임 — 재미를 느끼려면 먼저 메커니즘을 **공부해야** 했다. 실제로 붙잡힌 게임(vase=물 붓기, 2048=같은 수 합치기)은 **첫 3초에 뭘 하는지 자명**하고 깊이는 나중에 드러난다. 규칙: **깊이는 죽도록 단순한 첫 동작에서 "창발"해야지, 입장료로 요구하면 안 된다.** 인트로/설명을 읽어야 이해되는 코어 = kill 신호(GAME_PRINCIPLES "설명 없이 시작"과 정면).
- **깊이 = "화면의 시스템 수"가 아니라 "분당 의미있는 결정 수".** "누를 버튼이 바뀌는 게임"이 아니라 "뭘 누를지 고민하는 게임"을 만든다. 단 위 즉시성 규칙이 우선 — 고민이 "학습 후"에만 시작되면 실패.
