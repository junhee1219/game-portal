...
(() => {
  ...
  // 특수아이템(소용돌이) 글리프 — index.html #g-vortex 와 동일 path
  ...
  function drawAnimal(x, y, tier, angle, alpha, squash = 0, now = 0) {
    ...
    // 동물 글리프 (Path2D, 512 viewBox) — 따뜻한 짙은 갈색, 큼직하게
    const gp = GLYPH_P[def.g];
    if (gp) {
      ctx.save();
      const gs = (r * 1.62) / 512;           // 원 안에 꽉 차게 (더 큼직)
      ctx.scale(gs, gs);
      ctx.translate(-256, -256);
      ctx.fillStyle = 'rgba(86,54,34,0.95)'; // 따뜻한 짙은 갈색 (검정 아님)
      ctx.fill(gp);
      ctx.restore();
    }

    // 글로시 하이라이트: 윗쪽 둥근 빛 (사탕/스티커 광택) — 글리프 위에 살짝
    ctx.save();
